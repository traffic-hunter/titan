...
```bash
curl -LO https://github.com/traffic-hunter/titan/releases/download/0.8.3/titan-server-0.8.3.jar
```
...
Use the same `0.8.3` version for each artifact. The native client is selected
...
java -Dtitan.environment.path=./titan-env.yml -jar titan-server-0.8.3.jar
