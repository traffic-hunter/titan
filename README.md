<p align="center">
  <img src=".github/assets/titan-flow-lockup.svg" alt="Titan" width="420">
</p>
...
```bash
curl -LO https://github.com/traffic-hunter/titan/releases/download/0.8.4/titan-server-0.8.4.jar
```
...
```kotlin
implementation("org.traffichunter.titan:titan-spring-client:0.8.4")
```

For a standalone Java client:

```kotlin
implementation("org.traffichunter.titan:titan-client:0.8.4")
```
...
Use the same `0.8.4` version for each artifact. The native client is selected
...
java -Dtitan.environment.path=./titan-env.yml -jar titan-server-0.8.4.jar
